...
3. 에미일 검증 프로세스: 도메인 소유주 (고객) 에게 이메일 컨펌을 받아야 합니다.
...
1. AWS -> ACM 콘솔로 들어가서, 새 인증서를 요청한다.
...
4. 초기 발급 받은 인증서는 **검증 보류** 상태이다. 검증 보류를 해제하기 위해서는 화면처럼 도메인 이름과 값을 CNAME 타입으로 도메인 사업자에 A 레코드를 등록해 주어야 한다.
...
5. GABIA 도메인 사이트에 접속하여, 네임플러스 부가서비스로 들어간다.
...
6. GABIA 에 등록된 도메인에, CNAME 설정으로 들어간다.
...
7. 인증서의 A 레코드 도메인과 A 레코드 값을 차례로 넣어주고 등록하면 완성이다.
